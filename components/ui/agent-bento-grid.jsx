...
export function Card1() {
  ...
  return (
    <div
      className="w-full h-full relative overflow-hidden select-none bg-neutral-50 dark:bg-neutral-950/80 rounded-xl flex items-center justify-center p-2">
    ...
    <div className="w-full h-full flex flex-col gap-3.5 justify-between">
            ...
            <div key={i} className="flex-1 h-[76px] relative select-none">
              ...
              <motion.div
                ...
                <div className="flex flex-col min-w-0">
                  <span
                    className="text-[8px] text-muted-foreground/80 font-mono uppercase tracking-widest leading-none">{s.label}</span>
                  <span
                    className="text-base font-bold font-mono text-foreground leading-none mt-1.5 tracking-tight">{s.value}</span>
                  <div className="flex items-center gap-1.5 mt-2">
                    <span
                      className={`text-[8px] font-mono font-bold ${s.trend.startsWith("+") ? "text-emerald-500" : "text-rose-400"
                        }`}>
                      {s.trend}
                    </span>
      ...
      <div className="flex gap-2.5 px-0.5">
        {days.map((d, i) => (
          <p
            key={i}
            className="flex-1 text-center text-[8px] text-muted-foreground font-mono font-medium">{d}</p>
        ))}
      </div>
    </div>
  );
      ...
      {logs.map((l, i) => {
        ...
        return (
          <motion.div
            ...
            <div
              className={`w-full rounded-2xl border flex items-center gap-2.5 ${isActive
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span
                    className={`font-mono font-semibold text-foreground leading-none ${isActive ? "text-[10px]" : "text-[9px]"
                      }`}>{l.agent}</span>
                  <span
                    className={`font-mono uppercase tracking-wide rounded px-1 py-0.5 ${si.bg} ${si.color} ${isActive ? "text-[7px]" : "text-[6px]"
                      }`}>{l.status}</span>
                </div>
                {isActive && (
                  <p
                    className="text-[9px] text-muted-foreground truncate mt-0.5 leading-tight">{l.action}</p>
                )}
              </div>
  ...
  return (
    <div className="w-full h-full flex gap-5 py-2 px-3">
      ...
      <div className="flex-1 flex flex-col gap-0 min-w-0 pr-2">
        <p
          className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground mb-3">Namespaces</p>
          ...
          {namespaces.map((ns, i) => {
            ...
            return (
              <div key={ns.name} className="flex items-center gap-3 group relative">
                ...
                {/* Cyberpunk Bar track */}
                <div
                  className="flex-1 h-1.5 bg-muted/30 rounded-full overflow-hidden relative backdrop-blur-sm shadow-inner">
                  <motion.div
        ...
        <div className="flex items-center gap-2 pt-3 mt-auto">
          ...
          <span
            className="text-[8px] font-mono text-muted-foreground font-medium tracking-wide">Live retrieval active</span>
        </div>
      </div>
      {/* Thin divider */}
      ...
      <div className="w-[172px] shrink-0 flex flex-col gap-0">
        <p
          className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground mb-2.5">Retrieval Log</p>
            ...
            return (
              <motion.div
                ...
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span
                    className="text-[7px] font-mono text-muted-foreground/50 ml-auto tabular-nums">{q.t}</span>
                </div>
                <p
                  className="text-[8px] text-foreground/75 leading-tight font-mono truncate">{q.q}</p>
              </motion.div>
            );
          })}
        </div>
          ...
          <motion.div
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-col items-end gap-0.5 mt-0.5">
                ...
                <span
                  className="text-[7px] font-mono text-muted-foreground/80 uppercase tracking-widest leading-none">Calls</span>
              </div>
            </div>
            ...
            <div className="mt-2 flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <span
                  className="text-[10px] font-mono font-medium text-foreground tracking-tight">{t.name}</span>
                <span className="text-[8px] font-mono text-muted-foreground tabular-nums">{t.latency}</span>
              </div>
              <div
                className="w-full h-1.5 bg-muted/50 rounded-full overflow-hidden shadow-inner relative">
                <motion.div
